...
//printf("Gelezen shRNA:%s\n",item);
	...
	for (i=0;i<lib_file_count;i++) { // Assume lib_file is smallest
		...
		for (j=0;j<bin_file_count;j++) {
			distance = hamming_distance(bin_file_seq_array[j], lib_file_seq_array[i]);
...
if (distance<=distance_threshold) {
				sprintf(tmpbuf, "%s\t%s\t%i\t%s\t%s\t%i", barcode_array[j], bin_file_seq_array[j], bin_file_hits_array[j], lib_file_seq_array[i], lib_file_srid_array[i], distance);
