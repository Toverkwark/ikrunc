...
my $HomepagePrefix = "/media/Bastiaan_Portable/Work/Active\\ Projects/iKRUNC/software/ikrunc-website-full/";
my $HomepagePrefix2 = "/media/Bastiaan_Portable/Work/Active\ Projects/iKRUNC/software/ikrunc-website-full/";
...
my $WeighingFile = $opts{'w'};

#Assign proper genome id
$Species = lc($Species);
$RefSeqFile = "/media/Bastiaan_Portable/Work/Active\\ Projects/iKRUNC/software/RefSeq/human.txt";

#Filter the qualities.4 file for relevant target sites. As output, use a temporary file
my $timestamp = gettimeofday;
print "temp/" . $timestamp . ".svg";
`perl FilterAndSelect.pl -i "/media/Bastiaan_Portable/Work/Active Projects/iKRUNC/EGFR and SWISNF Library/lib2/p.qualities.4" -o $timestamp -c $ReportFractionOfTranscript -e $ReportNucleotidesAroundStartSite -n $ReportNumberOfNucleotides -r $RefSeq -s $Species`;

#When present, read in the weighing file
if ($WeighingFile) {
	open (WEIGHING,$WeighingFile) or die "ERROR in $ScriptName: Could not open weighing file $WeighingFile\n";
	while(defined(my $Line=<WEIGHING>)) {
		chomp($Line);
		my @Fields = split( /,/, $Line);
		$WeighingList{$Fields[0]}=$Fields[1];
	}
}
close (WEIGHING);
...
my $InputFile=$timestamp . ".seq";
open (IN, $InputFile) or die "ERROR in $ScriptName: Cannot open inputfile $InputFile\n";
open (OUT, ">", $timestamp . ".svg") or die "ERROR in $ScriptName: Cannot create svg file\n";
open (OUTHTML, ">", $timestamp . ".svg.html") or die "ERROR in $ScriptName: Cannot create svg html file\n";
...
foreach my $Orientation (keys %DisplayObjects) {
	foreach my $TargetCutSite (keys $DisplayObjects{$Orientation}) {
		...
		my $TriangleValue=-1;
		if ($WeighingFile) {		
			if($WeighingList{(substr($DisplayObjects{$Orientation}->{$TargetCutSite}->[5],0,20))}) {
				$TriangleValue=$WeighingList{(substr($DisplayObjects{$Orientation}->{$TargetCutSite}->[5],0,20))};
				$TriangleValue=$TriangleValue/333;
				my $Threshold=0;
				if($TriangleValue<$Threshold) {
					$TriangleValue=-2;
				}	
				else {
					$TriangleValue=($TriangleValue-$Threshold)/(1-$Threshold);
				}
			}
		}
		else {
			$TriangleValue=$DisplayObjects{$Orientation}->{$TargetCutSite}->[12];
		}
		$TriangleValue = int (511*($TriangleValue)) unless ($TriangleValue < 0);
		my $Blue = 255;
