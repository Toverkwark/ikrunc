...
while (defined(my $Line=<IN>)) {
	...
	if(!$FoundInPreviousRun || ($SkipLookingAtLast && $FoundInPreviousRun > -1) || ($FoundInPreviousRun == 1 && $Depth == 1)) { 
		...
		$ExonicRelativesFound{$TargetSequence} = 0;
		`perl "/media/Bastiaan_Portable/Work/Active Projects/iKRUNCH/software/scripts/FindRelativesOfOligo.pl" -i $TargetSequence -o $PotentialRelativesFile -d $Depth -s $SeedLength`;
	}
}
...
if ($ProcessRelatives) {
	`bowtie2 /media/Data/iKRUNCH/hg19-index/hg19 -f $PotentialRelativesFile -t --no-hd --score-min L,-5,0 -a -p 4 -S $RelativesMatchFile`;	
